...
const ProfileSkeleton = () => (
  <div className="space-y-6 p-4 sm:p-6 lg:p-8 max-w-[1400px] mx-auto">
    ...
    {/* Profile Header Card */}
    <div className="p-6 border bg-bg-canvas rounded-2xl border-border sm:p-8">
    ...
    {/* Settings Menu */}
    <div className="p-4 border bg-bg-canvas rounded-2xl border-border">
      {[1, 2, 3, 4, 5].map((i) => (
  ...
  return (
    <div className="flex items-center gap-3 p-3 transition-colors duration-200 rounded-lg group bg-light-bg-tertiary/50 dark:bg-dark-bg-tertiary/50 hover:bg-light-bg-tertiary dark:hover:bg-dark-bg-tertiary">
      <div className="p-2 transition-colors duration-200 rounded-lg bg-accent-primary/10 group-hover:bg-accent-primary/20">
        <Icon className="flex-shrink-0 w-4 h-4 text-primary" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-xs font-medium tracking-wide uppercase text-text-secondary">{label}</p>
        {href ? (
          <a
            href={href.startsWith("http") ? href : `https://${href}`}
            target="_blank"
            rel="noopener noreferrer"
            className="block text-sm font-medium truncate transition-colors text-primary hover:text-accent-primary-dark dark:hover:text-accent-primary-light"
          >
            {value}
          </a>
        ) : (
...
const StatCard = ({ icon: Icon, label, value, color, subtext }) => (
  <motion.div
    ...
    className="relative p-5 overflow-hidden transition-shadow duration-200 border group bg-bg-surface rounded-xl border-border hover:shadow-md dark:hover:shadow-dark-md"
  >
    <div
    ...
    <div className="relative">
      <div className="flex items-center justify-between mb-3">
        ...
        {subtext && (
          <span className="px-2 py-1 text-xs font-medium rounded-full text-text-secondary bg-bg-hover">
            {subtext}
          </span>
        )}
      </div>
      <p className="mb-1 text-3xl font-bold text-text-primary">{value}</p>
      <p className="text-xs font-medium tracking-wide uppercase text-text-secondary">{label}</p>
    </div>
  </motion.div>
)
...
const SettingsItem = ({ icon: Icon, title, subtitle, color, onClick, danger }) => (
  <motion.button
    ...
    whileTap={{ scale: 0.99 }}
    onClick={onClick}
    className={`w-full flex items-center gap-4 p-4 hover:bg-bg-hover transition-all duration-200 border-b border-border last:border-b-0 group ${danger ? "hover:bg-accent-danger/5 dark:hover:bg-accent-danger/10" : ""}`}
  >
    <div
      className={`p-2.5 rounded-xl ${danger ? "bg-accent-danger/10 group-hover:bg-accent-danger/20" : "bg-light-bg-hover group-hover:scale-110"} transition-all duration-200`}
    >
    ...
    <ChevronRight className="w-5 h-5 transition-colors text-text-secondary group-hover:text-accent-primary" />
  </motion.button>
)
...
const Me = () => {
  ...
  const displayName = capitalizeName(user?.fullname) || "User"
  ...
  const _avatarInitial = displayName.trim().charAt(0).toUpperCase()
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-6xl p-6 mx-auto space-y-6 md:p-8">
          ...
          <motion.div
            ...
            className="relative p-6 overflow-hidden border backdrop-blur-sm bg-bg-surface rounded-2xl border-border md:p-8"
          >
            {/* Decorative Element */}
            ...
            <div className="relative flex flex-col items-center gap-6 md:flex-row">
              ...
              <motion.div whileHover={{ scale: 1.05 }} className="relative group">
                ...
                >
                  <div className="relative flex items-center justify-center w-full h-full overflow-hidden text-5xl font-bold bg-transparent rounded-2xl text-text-primary">
                    {user?.avatar?.url &&
                    ...
                    !avatarFailed ? (
                      ...
                      <div className="flex items-center justify-center w-full h-full rounded-2xl bg-bg-elevated">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center transition-opacity duration-300 opacity-0 bg-utility-overlay group-hover:opacity-100">
                    <span className="text-sm font-medium text-text-inverse">Change</span>
                  </div>
                </label>
                <input
                  type="file"
                ...
                />
                {/* Online Status */}
                <div className="absolute w-6 h-6 border-4 rounded-full -bottom-1 -right-1 bg-success border-light-bg-secondary dark:border-dark-bg-tertiary" />
              </motion.div>

              {/* User Info */}
              <div className="flex-1 text-center md:text-left">
                <div className="flex flex-col gap-2 md:flex-row md:items-center md:gap-4">
                  <h2 className="text-3xl font-bold text-text-primary">{displayName}</h2>
                </div>
                <p className="flex items-center justify-center gap-2 mt-2 text-text-secondary md:justify-start">
              ...
              <div className="flex gap-2">
                <motion.button
                  ...
                  className="p-3 transition-colors border rounded-xl border-border hover:bg-bg-hover"
                >
                ...
                <motion.button
                  ...
                  className="p-3 transition-colors shadow-md rounded-xl bg-primary text-text-inverse hover:bg-accent-primary-dark"
                >
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            >
              {/* Profile Information */}
              <div className="overflow-hidden border bg-bg-surface rounded-2xl border-border">
                ...
                <div className="p-5">
                  ...
                  {user?.bio && (
                    <div className="p-4 mt-4 border rounded-xl bg-light-bg-tertiary/50 dark:bg-dark-bg-tertiary/50 border-border">
                      ...
                      <p className="text-sm leading-relaxed text-text-primary">{user.bio}</p>
                    </div>
                  )}
                </div>
              </div>
                  ...
                  {recentActivity.length === 0 ? (
                    <div className="flex items-center justify-center py-8">
                      <div className="text-center">
                        <Activity className="w-12 h-12 mx-auto mb-3 text-text-muted opacity-40" />
                        <p className="font-medium text-text-secondary">No recent activity</p>
                    ...
                    recentActivity.map((activity) => (
                      ...
                      >
                        <div className="flex-shrink-0 p-2 rounded-lg bg-bg-elevated">
            ...
            >
              ...
              {/* Danger Zone */}
              <div className="overflow-hidden border bg-bg-surface rounded-2xl border-accent-danger/30">
                ...
                <div className="p-5">
                  <div className="flex items-center justify-between">
                    ...
                    <motion.button
                      ...
                      className="flex items-center gap-2 px-4 py-2.5 rounded-xl border-2 border-danger text-danger font-semibold hover:bg-danger hover:text-light-text-inverse transition-all duration-200"
                    >
          ...
          >
            <p className="text-sm font-medium text-text-muted">TaskFlow v1.0.0</p>
            <p className="mt-1 text-xs text-text-muted">Work that feels human</p>
          </motion.div>
        </div>
      </div>
      ...
      {showPersonalInfo && (
        ...
        >
          <motion.div
            ...
            className="bg-bg-surface rounded-2xl border border-border p-6 max-w-lg w-full mx-4 shadow-2xl max-h-[80vh] overflow-y-auto"
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-text-muted">Your profile details</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setIsEditModalOpen(true)}
                  className="p-2 transition-colors rounded-lg hover:bg-bg-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30"
                  title="Edit Profile"
                >
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4 p-4 rounded-xl bg-light-bg-tertiary/50 dark:bg-dark-bg-tertiary/50">
                <div className="flex items-center justify-center flex-shrink-0 w-16 h-16 text-2xl font-bold rounded-xl bg-gradient-to-br from-accent-primary via-accent-success to-accent-warning text-text-inverse">
                  {user?.avatar?.url ? (
                    <img
                      src={user.avatar.url}
                      alt="Avatar"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <InfoItem icon={Briefcase} label="Job Title" value={user?.jobTitle || "Not set"} />
              ...
              <div className="grid grid-cols-2 gap-3">
                {user?.website && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-3 p-3 transition-colors rounded-xl bg-light-bg-tertiary/50 dark:bg-dark-bg-tertiary/50 hover:bg-bg-hover"
                  >
                    <Globe className="flex-shrink-0 w-5 h-5 text-primary" />
                ...
                {user?.linkedin && (
                  <a
                    ...
                    className="flex items-center gap-3 p-3 transition-colors rounded-xl bg-light-bg-tertiary/50 dark:bg-dark-bg-tertiary/50 hover:bg-bg-hover"
                  >
                    <Linkedin className="flex-shrink-0 w-5 h-5 text-primary" />
                    <span className="text-sm font-medium truncate text-text-primary">LinkedIn</span>
                  </a>
                )}
                {user?.github && (
                  <a
                    ...
                    className="flex items-center gap-3 p-3 transition-colors rounded-xl bg-light-bg-tertiary/50 dark:bg-dark-bg-tertiary/50 hover:bg-bg-hover"
                  >
                    <Github className="flex-shrink-0 w-5 h-5 text-primary" />
                    <span className="text-sm font-medium truncate text-text-primary">GitHub</span>
                  </a>
                )}
              </div>
      ...
      {activeSetting && (
        ...
        >
          <motion.div
            initial={{ scale: 0.95, opacity: 0, y: 20 }}
            animate={{ scale: 1, opacity: 1, y: 0 }}
            className="w-full max-w-md p-6 mx-4 border shadow-2xl bg-bg-surface rounded-2xl border-border"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-text-primary">{activeSetting.title}</h3>
                  <p className="text-sm text-text-muted">{activeSetting.subtitle}</p>
                </div>
              </div>
              <button
                onClick={() => setActiveSetting(null)}
            ...
            <div className="space-y-4">
              {activeSetting.title === "Notifications" && (
                <>
                  <div className="flex items-center justify-between p-4 rounded-xl bg-light-bg-tertiary/50 dark:bg-dark-bg-tertiary/50">
                    ...
                    <div className="relative w-12 h-6 rounded-full cursor-pointer bg-primary">
                  ...
                  <div className="flex items-center justify-between p-4 rounded-xl bg-light-bg-tertiary/50 dark:bg-dark-bg-tertiary/50">
                    <div>
                      ...
                      <p className="text-sm text-text-muted mt-0.5">Get instant alerts</p>
                    </div>
              ...
              {activeSetting.title === "Appearance" && (
                <>
                  <p className="mb-3 text-sm text-text-secondary">Choose your preferred theme</p>
                  <div className="grid grid-cols-3 gap-3">
                    <button
                      type="button"
                      onClick={() => handleThemeChange("light")}
                      className={`p-4 rounded-xl border-2 ${selectedTheme === "light" ? "border-accent-primary bg-bg-canvas dark:bg-dark-bg-primary" : "border-light-border dark:border-dark-border"} focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30`}
                    >
                      <div className="w-full h-12 mb-2 rounded-lg bg-bg-surface" />
                      <p className="text-xs font-medium text-center text-text-primary">Light</p>
                    </button>
                    <button
                      type="button"
                      onClick={() => handleThemeChange("dark")}
                      className={`p-4 rounded-xl border-2 ${selectedTheme === "dark" ? "border-accent-primary bg-dark-bg-primary" : "border-light-border dark:border-dark-border"} focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30`}
                    >
                      <div className="w-full h-12 mb-2 rounded-lg bg-dark-bg-tertiary" />
                      <p className="text-xs font-medium text-center text-text-primary">Dark</p>
                    </button>
                    <button
                      type="button"
                      onClick={() => handleThemeChange("system")}
                      className={`p-4 rounded-xl border-2 ${selectedTheme === "system" ? "border-accent-primary" : "border-light-border dark:border-dark-border"} focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30`}
                    >
                      <div className="w-full h-12 mb-2 rounded-lg bg-gradient-to-r from-light-bg-secondary to-dark-bg-tertiary" />
                      <p className="text-xs font-medium text-center text-text-primary">System</p>
                    </button>
                  </div>
                </>
              )}
              ...
              {activeSetting.title === "Security" && (
                <>
                  <div className="p-4 rounded-xl bg-light-bg-tertiary/50 dark:bg-dark-bg-tertiary/50">
                    <p className="mb-1 font-medium text-text-primary">Password</p>
                    <p className="text-sm text-text-muted">Last changed 30 days ago</p>
                  </div>
                  <button className="w-full p-4 text-left rounded-xl p-6 bg-bg-surface border border-border rounded-xl shadow-sm hover:shadow-md hover:border-primary/40 transition-all cursor-pointer">
                    <p className="font-medium text-text-primary">Change Password</p>
                    <p className="text-sm text-text-muted mt-0.5">Update your password</p>
                  </button>
                  <button className="w-full p-4 text-left rounded-xl p-6 bg-bg-surface border border-border rounded-xl shadow-sm hover:shadow-md hover:border-primary/40 transition-all cursor-pointer">
                    <p className="font-medium text-text-primary">Two-Factor Authentication</p>
                    <p className="text-sm text-text-muted mt-0.5">Add an extra layer of security</p>
                  </button>
                </>
              )}
              ...
              {activeSetting.title === "Preferences" && (
                <div className="space-y-3">
                  <div>
                    ...
                    <select className="w-full px-4 py-3 bg-bg-surface text-text-primary border border-border rounded-lg placeholder:text-text-muted focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all">
            ...
            <div className="flex gap-3 pt-6 mt-6 border-t border-border">
              <button
                ...
                className="bg-primary hover:bg-primary/90 text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all cursor-pointer flex-1 px-4 py-2.5 rounded-lg"
              >
                Save Changes
              </button>
              <button
                onClick={() => setActiveSetting(null)}
                className="bg-transparent hover:bg-bg-hover text-text-secondary focus:outline-none transition-all cursor-pointer flex-1 px-4 py-2.5 rounded-lg"
              >
                Cancel
              </button>
            </div>
      ...
      {showDeleteConfirm && (
        ...
        >
          <motion.div
            ...
            className="w-full max-w-sm p-6 mx-4 border shadow-xl bg-bg-surface rounded-2xl border-danger dark:shadow-dark-lg"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setShowDeleteConfirm(false)}
                className="flex-1 px-4 py-2.5 rounded-lg border border-border hover:bg-bg-hover text-text-primary transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                disabled={deleting}
                className="bg-danger hover:bg-danger/90 text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-danger/50 transition-all cursor-pointer flex-1 px-4 py-2.5 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed"
              >
