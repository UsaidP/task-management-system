...
const ProjectPage = () => {
	...
	const handleTaskUpdated = (updatedTask) => {
		if (!updatedTask || !updatedTask.status || !columns[updatedTask.status])
			return;
		setColumns((prevColumns) => {
			const newColumns = { ...prevColumns };
			...
			if (originalStatus && originalStatus !== updatedTask.status) {
				const oldTasks = newColumns[originalStatus].tasks.filter(
		...
		>
			<header className="flex items-center justify-between mb-8">
				{loading ? (
					...
					>
						<h1 className="text-4xl font-bold text-bento-text-primary mb-2">
							{project?.name}
						</h1>
						<p className="text-lg text-bento-text-secondary">
							{project?.description}
						</p>
					</motion.div>
				)}
			...
			>
				<div>
					<p>
						Are you sure you want to delete the task titled "
						{selectedTask?.title}"?
					</p>
