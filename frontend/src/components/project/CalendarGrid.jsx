  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-bg-surface">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-3 sm:p-4 border-b border-border bg-bg-surface shrink-0">
        <div>
          <h2 className="text-lg sm:text-xl font-semibold text-text-primary">Calendar</h2>
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          <div className="flex items-center gap-1">
            ...
            <span className="text-xs sm:text-sm font-semibold text-text-primary min-w-[120px] sm:min-w-[160px] text-center px-2">
          ...
          <button
            ...
            className="px-3 py-1.5 text-xs sm:text-sm rounded-lg bg-bg-surface border border-border text-text-primary hover:bg-bg-hover transition-colors min-h-[36px]"
          >
            Today
          </button>
        </div>
      ...
      <div className="flex-1 overflow-auto p-3 sm:p-4">
        <div className="bg-bg-canvas rounded-lg border border-border overflow-hidden">
          <div className="grid grid-cols-7 text-center text-xs font-semibold text-text-muted uppercase border-b border-border min-w-[700px]">
            {weekDays.map((day) => (
              <div key={day} className="py-2 sm:py-3 bg-bg-surface">
                {day}
              </div>
            ))}
          </div>
          <div className="grid grid-cols-7 min-w-[700px]">
            {calendarDaysData.map((item, index) => {
              ...
              return (
                <div
                  key={item.day}
                  className={`min-h-[80px] sm:min-h-[100px] p-1.5 sm:p-2 border-b border-r border-border last:border-r-0 ${isToday ? "bg-accent-primary/10 dark:bg-accent-primary/20" : "hover:bg-light-bg-hover/50 dark:hover:bg-dark-bg-hover/50"}`}
                >
                  <div
                    className={`flex items-center justify-center w-5 h-5 sm:w-6 sm:h-6 rounded-full text-xs sm:text-xs font-medium mb-1.5 ${isToday ? "bg-accent-primary text-white" : "text-light-text-tertiary dark:text-dark-text-tertiary"}`}
                  >
                    {item.day}
                  </div>
                      ...
                      <>
                        {dayTasks.slice(0, 2).map((task) => (
                          <motion.div
                            ...
                            className="text-[10px] sm:text-xs p-1 rounded truncate cursor-pointer bg-bg-surface text-text-secondary hover:bg-bg-hover transition-colors"
                          >
                            <div className="font-medium truncate">{task.title}</div>
                          </motion.div>
                        ))}
                        {dayTasks.length > 2 && (
                          <div className="text-[10px] sm:text-xs text-text-muted pl-1">
