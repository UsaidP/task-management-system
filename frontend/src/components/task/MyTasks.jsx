...
export const MyTasks = () => {
  ...
  const getPriorityBadgeClass = (priority) => {
    switch (priority) {
      ...
      default: return "bg-light-bg-hover dark:bg-dark-bg-hover text-light-text-tertiary dark:text-dark-text-tertiary px-3 py-1 rounded-full text-xs font-medium"
    }
  }

  const getStatusBadgeClass = (status) => {
    switch (status) {
      case "completed": return "badge-status-done"
  ...
  return (
    <div className="h-full flex flex-col" style={{ boxShadow: "0px 0px 1px 0.1px #000000" }}>
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4 items-center p-6 bg-light-bg-secondary dark:bg-dark-bg-secondary shrink-0">
        <div>
          <h1 className="text-3xl font-serif font-bold text-light-text-primary dark:text-dark-text-primary mb-2">My Tasks</h1>
          <p className="text-light-text-secondary dark:text-dark-text-secondary">Track and manage all your assigned work across projects.</p>
        </div>
        <button className="btn-primary whitespace-nowrap flex items-center gap-2" onClick={() => setCreateModalOpen(true)}>
      ...
      {/* Toolbar */}
      <div className="flex-1 overflow-y-auto p-6 pt-0 bg-light-bg-primary dark:bg-dark-bg-primary">
        <div className="w-full md:flex-1 relative">
          ...
          <input
            ...
            className="w-full bg-light-bg-secondary dark:bg-dark-bg-tertiary border border-light-border dark:border-dark-border rounded-lg px-4 py-2 pl-10 text-sm focus:outline-none focus:border-accent-primary text-light-text-primary dark:text-dark-text-primary placeholder:text-light-text-tertiary w-full"
          />
        </div>
          ...
          >
                ...
                <Listbox.Options className="absolute z-50 mt-1 w-full bg-light-bg-secondary dark:bg-dark-bg-tertiary border border-light-border dark:border-dark-border rounded-lg shadow-lg overflow-hidden">
                  {["all", "todo", "in-progress", "under-review", "completed"].map((status) => (
                    <Listbox.Option key={status} value={status} as={Fragment}>
                      {({ active }) => (
                        <li className={`px-3 py-2 cursor-pointer text-sm ${active ? "bg-accent-primary text-white" : "text-light-text-primary dark:text-dark-text-primary"}`}>
              ...
              <div className="relative">
                <Listbox.Button className="bg-light-bg-secondary dark:bg-dark-bg-tertiary border border-light-border dark:border-dark-border rounded-lg px-3 py-2 text-sm text-left min-w-[150px] max-w-[180px] cursor-pointer hover:bg-light-bg-hover dark:hover:bg-dark-bg-hover transition-colors">
                  <span className="text-light-text-primary dark:text-dark-text-primary truncate block">
                    {projectFilter === "all" ? "All Projects" : projects.find(p => p._id === projectFilter)?.name || "All Projects"}
                  </span>
                  <FiChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 w-4 h-4 text-light-text-tertiary" />
                </Listbox.Button>
                <Listbox.Options className="absolute z-50 mt-1 w-full bg-light-bg-secondary dark:bg-dark-bg-tertiary border border-light-border dark:border-dark-border rounded-lg shadow-lg overflow-hidden max-h-60 overflow-auto">
                  ...
                  {projects.map((p) => (
                    <Listbox.Option key={p._id} value={p._id} as={Fragment}>
                      {({ active }) => (
                        <li className={`px-3 py-2 cursor-pointer text-sm truncate ${active ? "bg-accent-primary text-white" : "text-light-text-primary dark:text-dark-text-primary"}`}>
                          {p.name}
                        </li>
                      )}
                    </Listbox.Option>
            ...
            <Listbox value={sprintFilter} onChange={setSprintFilter}>
              <div className="relative">
                <Listbox.Button className="bg-light-bg-secondary dark:bg-dark-bg-tertiary border border-light-border dark:border-dark-border rounded-lg px-3 py-2 text-sm text-left min-w-[120px] cursor-pointer hover:bg-light-bg-hover dark:hover:bg-dark-bg-hover transition-colors">
                  <span className="text-light-text-primary dark:text-dark-text-primary">
                    {sprintFilter === "all" ? "All Sprints" : SPRINTS.find(s => s.id === sprintFilter)?.name || "All Sprints"}
                  </span>
                  <FiChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 w-4 h-4 text-light-text-tertiary" />
                </Listbox.Button>
                <Listbox.Options className="absolute z-50 mt-1 w-full bg-light-bg-secondary dark:bg-dark-bg-tertiary border border-light-border dark:border-dark-border rounded-lg shadow-lg overflow-hidden">
      ...
      <div className="flex-1 bg-light-bg-secondary dark:bg-dark-bg-tertiary rounded-xl border border-light-border dark:border-dark-border shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="flex border-b border-light-border dark:border-dark-border bg-light-bg-hover/50 dark:bg-dark-bg-hover/30 p-4 font-semibold text-sm text-light-text-tertiary dark:text-dark-text-tertiary">
          ...
          <div
            className="w-32 flex items-center justify-start gap-1 cursor-pointer hover:text-light-text-primary dark:hover:text-dark-text-primary transition-colors"
          >
            Status
          </div>
          <div
          ...
          <div
            className="w-48 hidden md:flex items-center justify-start gap-1 cursor-pointer hover:text-light-text-primary dark:hover:text-dark-text-primary transition-colors select-none"
          >
            Project
          </div>
          <div
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            <div className="p-4 space-y-4">
              {[1, 2, 3, 4, 5].map(i => (
                <div key={i} className="h-12 bg-light-bg-hover dark:bg-dark-bg-hover rounded-lg animate-pulse" />
              ))}
            </div>
              ...
              {filteredAndSortedTasks.map(task => (
                ...
                >
                  <div className="flex-1 min-w-[300px] flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-light-text-primary dark:text-dark-text-primary text-sm line-clamp-1">{task.title}</p>
                    </div>
                  </div>
                  ...
                  <div className="w-32 flex items-center text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary">
                    {task.dueDate ? (
                      <span className={`flex items-center gap-1.5 ${dayjs(task.dueDate).isBefore(dayjs(), 'day') && task.status !== 'completed' ? 'text-accent-danger' : ''}`}>
                        <FiCalendar className="w-4 h-4" />
                        {dayjs(task.dueDate).format('MMM D')}
                      </span>
                    ) : (
                      <span className="text-light-text-tertiary dark:text-dark-text-tertiary ml-6">—</span>
                    )}
                  </div>
                </motion.div>
              ))}
