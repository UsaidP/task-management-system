...
import { ActivityIcon, TriangleAlertIcon, ClipboardIcon, EyeIcon, PlusIcon } from "@animateicons/react/lucide"
import { CalendarIcon, SquareCheckIcon, ClockIcon } from "lucide-react"
...
const OverviewSkeleton = () => (
  ...
  >
    <div className="flex items-center justify-between animate-pulse">
      <div>
        <div className="w-64 h-10 mb-3 rounded-xl bg-light-bg-hover dark:bg-dark-bg-hover" />
        <div className="h-6 rounded-xl w-80 bg-light-bg-hover dark:bg-dark-bg-hover" />
      </div>
      <div className="hidden w-48 h-6 rounded-xl bg-light-bg-hover dark:bg-dark-bg-hover md:block" />
    </div>
...
const StatCard = memo(({ icon, label, value, color, delay = 0 }) => (
  <motion.div
    ...
    className="relative overflow-hidden transition-all duration-300 card group interactive-card"
  >
    ...
    <div className="relative">
      ...
      <div>
        <p className="py-1 mb-1 font-sans text-4xl font-bold tracking-tight text-light-text-primary dark:text-dark-text-primary">
          {value}
        </p>
...
const RecentActivityItem = memo(({ task, onClick, index }) => (
  ...
  >
    ...
    <div className="flex-1 min-w-0">
      <h3 className="mb-1 text-sm font-semibold text-light-text-primary dark:text-dark-text-primary line-clamp-1">
        {task.title}
      </h3>
      <p className="mb-2 text-xs text-light-text-secondary dark:text-dark-text-secondary line-clamp-1">
        {task.description}
      </p>
      <span className="inline-block text-[10px] uppercase font-bold px-2 py-0.5 rounded-full bg-light-bg-hover dark:bg-dark-bg-hover text-light-text-tertiary dark:text-dark-text-tertiary tracking-wider">
        {task.status.replace("-", " ")}
      </span>
    </div>
    <div className="flex-shrink-0 text-xs text-light-text-tertiary dark:text-dark-text-tertiary whitespace-nowrap mt-0.5">
...
const UpcomingTaskItem = memo(({ task, onClick, index }) => {
  ...
  return (
    <motion.button
      ...
      className={`flex items-center justify-between w-full p-4 rounded-xl border transition-all duration-200 group mb-3 ${
        isUrgent
          ? "border-accent-danger/30 bg-accent-danger/5 hover:bg-accent-danger/10"
          : "border-light-border/60 dark:border-dark-border/60 hover:border-light-border dark:hover:border-dark-border hover:bg-light-bg-secondary/50 dark:hover:bg-dark-bg-secondary/50"
      } hover:shadow-sm active:scale-[0.99] focus:outline-none focus:ring-2 focus:ring-accent-primary/30 cursor-pointer text-left`}
    >
      <div className="flex items-center gap-3">
        <div
          className={`p-2.5 rounded-xl transition-all duration-200 group-hover:scale-105 ${isUrgent ? "bg-accent-danger text-white shadow-sm shadow-accent-danger/20" : "bg-light-bg-hover dark:bg-dark-bg-hover text-light-text-tertiary dark:text-dark-text-tertiary"}`}
        >
          <CalendarIcon className="w-4 h-4" />
        </div>
        <div className="min-w-0">
          <h3 className="text-sm font-semibold text-light-text-primary dark:text-dark-text-primary line-clamp-1">
            {task.title}
          </h3>
      ...
      <div
        className={`text-sm font-medium px-3 py-1 rounded-lg ${isUrgent ? "text-accent-danger bg-accent-danger/10" : "text-light-text-primary dark:text-dark-text-primary bg-light-bg-secondary dark:bg-dark-bg-tertiary"}`}
      >
  ...
  return (
    <div className="space-y-8">
      ...
      {user ? (
        ...
        >
          <div>
            <h1 className="mb-2 font-serif text-3xl font-bold tracking-tight sm:text-4xl text-light-text-primary dark:text-dark-text-primary">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center px-4 py-2.5 text-sm font-medium border bg-light-bg-secondary dark:bg-dark-bg-tertiary rounded-xl text-light-text-secondary dark:text-dark-text-secondary border-light-border/60 dark:border-dark-border/60">
              <CalendarIcon className="w-4 h-4 mr-2 text-accent-primary" />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <h2 className="flex items-center font-serif text-lg font-bold tracking-tight text-light-text-primary dark:text-dark-text-primary">
                <CalendarIcon className="w-5 h-5 mr-2.5 text-accent-primary" />
                Upcoming Deadlines
              </h2>
              <Link
                to="/my-tasks"
                aria-label="View all tasks"
                className="text-sm font-medium transition-colors duration-150 text-accent-primary hover:text-accent-primary-dark"
              >
                View All →
              </Link>
            </div>
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <h2 className="flex items-center font-serif text-lg font-bold tracking-tight text-light-text-primary dark:text-dark-text-primary">
                <ActivityIcon className="w-5 h-5 mr-2.5 text-accent-warning" />
                Current Sprint Progress
              </h2>
              <span className="px-3 py-1 text-xs font-semibold border rounded-lg bg-accent-warning/10 text-accent-warning border-accent-warning/20">
                Sprint 4: Q3 Goals
              </span>
            </div>
            <div className="space-y-4">
              <div className="flex items-end justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-2xl font-bold tracking-tight text-light-text-primary dark:text-dark-text-primary">
                    68%
                  </p>
                </div>
              ...
              >
                ...
                <div
                  className="absolute inset-y-0 left-0 w-full shimmer"
                  style={{ width: "68%" }}
                />
              </div>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-light-border/40 dark:border-dark-border/40">
                ...
                <p className="text-sm font-semibold text-accent-success">{completionRate}%</p>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="lg:col-span-1">
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <h2 className="flex items-center font-serif text-lg font-bold tracking-tight text-light-text-primary dark:text-dark-text-primary">
                <ClockIcon className="w-5 h-5 mr-2.5 text-accent-success" />
                Recent Activity
              </h2>
            ...
            {loading ? (
              ...
              <div className="py-12 text-center">
                ...
                <p className="mt-1 text-xs text-light-text-tertiary dark:text-dark-text-tertiary">
                  Tasks you work on will appear here.
                </p>
              </div>
            )}
            ...
            <button
              ...
              className="w-full py-3 mt-6 text-sm font-semibold transition-all duration-200 border rounded-xl text-light-text-secondary dark:text-dark-text-secondary border-light-border/60 dark:border-dark-border/60 hover:bg-light-bg-hover dark:hover:bg-dark-bg-hover hover:border-light-border dark:hover:border-dark-border active:scale-[0.99] focus:outline-none focus:ring-2 focus:ring-accent-primary/20"
            >
              View Activity Log
            </button>
          </motion.div>
