      ...
      {({ isActive }) => (
        <>
          ...
          <div className={`flex items-center justify-center ${isCollapsed ? "w-full" : ""}`}>
            <Icon
              className={`w-[18px] h-[18px] flex-shrink-0 transition-all duration-200 ${isCollapsed ? "" : "mr-3"} ${isActive ? "scale-110" : "group-hover:scale-105"}`}
              aria-hidden="true"
            />
          </div>
...
const Sidebar = () => {
  ...
  const renderProjectList = () => {
    ...
    if (error) return <div className="px-4 py-2 text-xs text-danger">Failed to load projects</div>
      ...
      return (
        ...
        >
          <button
            ...
            className={`flex items-center w-full px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200 group border ${isActive ? "bg-accent-primary/12 border-accent-primary/30 text-primary dark:bg-accent-primary/20 dark:border-accent-primary/40 shadow-sm" : "border-transparent text-text-primary hover:bg-bg-hover hover:border-light-border dark:hover:border-dark-border"} ${isCollapsed ? "justify-center px-0 w-10 h-10 mx-auto !rounded-lg !p-0" : ""}`}
          >
            <div className={`flex items-center justify-center ${isCollapsed ? "w-full" : ""}`}>
              <span
                className={`w-2.5 h-2.5 rounded-full transition-all duration-200 flex-shrink-0 ${isActive ? "bg-accent-primary shadow-sm shadow-accent-primary/40 scale-110" : "bg-light-text-secondary/50 dark:bg-dark-text-secondary/50 group-hover:bg-accent-primary/70 group-hover:scale-110"} ${isCollapsed ? "" : "mr-3"}`}
                style={
                  isActive
                    ? {
                        transitionTimingFunction: "cubic-bezier(0.34, 1.56, 0.64, 1)",
            ...
            {!isCollapsed && (
              <span className="font-medium truncate text-text-primary">{project.name}</span>
            )}
          </button>
        </motion.div>
      )
      ...
      <>
        ...
        {!isCollapsed && projectFilter && (
          <motion.button
            ...
            className="flex items-center w-full px-3 py-2 mb-2 text-sm font-medium transition-all border rounded-xl text-primary hover:bg-accent-primary/10 border-accent-primary/20 hover:border-accent-primary/40 focus:outline-none focus:ring-2 focus:ring-accent-primary/20"
          >
        ...
        {!isCollapsed && recent.length > 0 && (
          <div className="mb-3">
            <p className="px-3 py-1.5 text-[10px] font-bold text-text-muted uppercase tracking-widest">
              Recent
            </p>
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className={`fixed top-0 left-0 h-dvh bg-bg-canvas border-r border-light-border/70 dark:border-dark-border/70 z-50 flex flex-col pt-4 transition-all duration-300 ${isDesktop ? (isCollapsed ? "w-20" : "w-72") : "w-72"}`}
        style={
      ...
      >
        ...
        <div className="flex items-center justify-between flex-none px-4 pb-4 border-b border-light-border/60 dark:border-dark-border/60">
          ...
          {!isDesktop && (
            <button
              ...
              className="p-2 transition-all duration-150 rounded-xl text-text-muted hover:text-accent-danger hover:bg-bg-hover active:scale-90 focus:outline-none focus:ring-2 focus:ring-accent-primary/20"
            >
          ...
          {isDesktop && (
            <button
              ...
              className="absolute z-10 p-1.5 flex items-center justify-center transition-all duration-200 -right-3 top-5 rounded-lg bg-bg-canvas border border-border shadow-sm hover:shadow-md hover:scale-105 text-text-muted hover:text-accent-primary hover:bg-bg-hover focus:outline-none focus:ring-2 focus:ring-accent-primary/20 active:scale-95"
            >
              {isCollapsed ? (
          ...
          <div className="px-4 py-3 border-b border-light-border/60 dark:border-dark-border/60">
            ...
            <div className="relative">
              ...
              >
                ...
                <ChevronDown
                  className={`w-4 h-4 text-text-muted group-hover:text-light-text-primary transition-transform duration-200 ${isSprintMenuOpen ? "rotate-180" : ""}`}
                  aria-hidden="true"
                />
              </button>
              ...
              <AnimatePresence>
                {isSprintMenuOpen && projectFilter && (
                  <motion.div
                    ...
                    className="absolute left-0 right-0 z-50 mt-1 overflow-hidden border shadow-lg rounded-xl top-full bg-bg-canvas border-border"
                  >
                    <button
                      onClick={() => {
                        setSprintFilter(null)
                      ...
                      className={`w-full text-left px-3 py-2.5 text-sm hover:bg-bg-hover transition-colors focus:outline-none focus:ring-2 focus:ring-inset focus:ring-accent-primary/20 ${!sprintFilter ? "text-accent-primary font-medium bg-accent-primary/5" : "text-light-text-primary dark:text-dark-text-primary"}`}
                    >
                      All Sprints
                    </button>
                    {sprintsLoading ? (
                      ...
                      <div className="px-3 py-2 text-xs text-text-muted">No sprints</div>
                    ) : (
                      sprints.map((sprint) => (
                        <button
                          key={sprint._id}
                          ...
                          className={`w-full text-left px-3 py-2.5 text-sm hover:bg-bg-hover transition-colors focus:outline-none focus:ring-2 focus:ring-inset focus:ring-accent-primary/20 ${sprintFilter === sprint._id ? "text-accent-primary font-medium bg-accent-primary/5" : "text-light-text-primary dark:text-dark-text-primary"}`}
                        >
                          {sprint.name}
                        </button>
                      ))
        ...
        <div className="flex-1 px-3 py-4 space-y-6 overflow-y-auto">
          ...
          <div>
            {!isCollapsed && (
              <p className="px-3 mb-2 text-[11px] font-bold text-text-secondary uppercase tracking-widest">
                Views
              </p>
            )}
            ...
            {!isCollapsed ? (
              <div className="flex items-center justify-between px-3 mb-2">
                <p className="text-[11px] font-bold text-text-secondary uppercase tracking-widest flex items-center gap-1.5">
                  ...
                  {!isLoading && !error && myProjects.length > 0 && (
                    <span className="bg-accent-primary/10 text-primary dark:bg-accent-primary/20 px-1.5 py-0.5 rounded-full text-[10px] font-bold">
                      {myProjects.length}
                    </span>
                  )}
                </p>
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className={`p-1 text-text-secondary flex justify-center items-center hover:text-accent-primary hover:bg-bg-hover rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-accent-primary/20 ${isProjectMenuOpen ? "" : "-rotate-180"}`}
                  >
                  ...
                  <button
                    ...
                    className="p-1 transition-all duration-150 rounded-lg text-text-secondary hover:text-accent-primary hover:bg-bg-hover active:scale-90 focus:outline-none focus:ring-2 focus:ring-accent-primary/20"
              ...
              <div className="flex justify-center mb-2">
                <button
                  onClick={() => setIsModalOpen(true)}
                  aria-label="Create project"
                  className="p-2 transition-all duration-150 rounded-lg text-text-muted hover:text-accent-primary hover:bg-bg-hover active:scale-90 focus:outline-none focus:ring-2 focus:ring-accent-primary/20"
                  title="Create Project"
                >
