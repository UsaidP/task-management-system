...
describe("deleteComment Controller Logic", () => {
		...
		it("should handle null/undefined taskId", () => {
			const validateTaskId = (taskId) => {
				if (!taskId || !mongoose.Types.ObjectId.isValid(taskId)) {
					return { error: "Invalid task ID", valid: false }
				}
				return { valid: true }
			}
		...
		it("should handle null/undefined commentId", () => {
			const validateCommentId = (commentId) => {
				if (!commentId || !mongoose.Types.ObjectId.isValid(commentId)) {
					return { error: "Invalid comment ID", valid: false }
				}
				return { valid: true }
			}
	...
	describe("Comment Deletion Logic", () => {
		...
		it("should handle removing non-existent comment", () => {
			const comments = [{ _id: new mongoose.Types.ObjectId(), text: "comment 1" }]
	...
	describe("Error Conditions", () => {
		it("should return correct error codes for each failure case", () => {
			const errorMapping = {
				commentNotFound: { message: "Comment not found", statusCode: 404 },
				invalidCommentId: { message: "Invalid comment ID", statusCode: 400 },
				invalidTaskId: { message: "Invalid task ID", statusCode: 400 },
				missingCommentId: { message: "Invalid comment ID", statusCode: 400 },
				missingTaskId: { message: "Invalid task ID", statusCode: 400 },
				notOwner: { message: "You can only delete your own comments", statusCode: 403 },
				taskNotFound: { message: "Task not found", statusCode: 404 },
			}
